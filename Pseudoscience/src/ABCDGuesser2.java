...
public final class ABCDGuesser2 {
    ...
    private static double[] formulaExponents(double microOne, double w,
        ...
        /**
         * For loops inside for loops that will try to find and will approximate
         * a,b,c,d values that are the closet to μ.
         */
        for (int i = 0; i < exponentsArray.length; i++) {
            double a = exponentsArray[i];
            for (int j = 0; j < exponentsArray.length; j++) {
                double b = exponentsArray[j];
                for (int k = 0; k < exponentsArray.length; k++) {
                    double c = exponentsArray[k];
                    for (int l = 0; l < exponentsArray.length; l++) {
