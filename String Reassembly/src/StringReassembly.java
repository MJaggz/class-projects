...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //Initialize a string variable to combine str1 and str2 not combining
        //the overlap part.
        resultCombination = str1.substring(0, str1.length() - overlap);
        //At the end, return the combined string.
        return resultCombination + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //Make a boolean condition named foundSubstring and set it to false
        //to start with.
        boolean foundSubstring = false;
        //Initialize a new set that will hold the strings that wer removed from
        //strSet.
        Set<String> removeSubstring = new Set1L<String>();
        //Used a for each loop to check if the string str is inside of item and
        //if it is, set foundSubstring to true.
        for (String item : strSet) {
            if (item.indexOf(str) >= 0) {
                foundSubstring = true;
            }
            if (str.indexOf(item) >= 0) {
                //Check if item is a substring of str and if it is, add it to the
                //removeSubstring set.
                removeSubstring.add(item);
            }
        }
        ///Used a for each loop to look for substrings of str and remove them
        //from the item.
        for (String item : removeSubstring) {
            strSet.remove(item);
        }
        //If the string str is not a substring and has no duplicates, add it
        //to Set.
        if (!foundSubstring && !strSet.contains(str)) {
            strSet.add(str);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //I made a new set called inputSetResult.
        Set<String> inputSetResult = new Set1L<>();
        //While loop that will check if the input is at the end of the string.
        //If it is not, store that line.
        while (!input.atEOS()) {
            String inputLine = input.nextLine();
            //USe the addToSetAvoidingSubstrings method to look for duplicats
            //and substrings.
            addToSetAvoidingSubstrings(inputSetResult, inputLine);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //Initialize a new string called StringText.
        String stringText = text;
        ///Use a for loop to check each single char of the text string.
        for (int i = 0; i < stringText.length(); i++) {
            //If that char is a '~', make a new line.
            if (stringText.charAt(i) == '~') {
                out.println();

            } else {
                //If the char is not a '~', leave it as the char it is and print
                //it out.
                char characterPosition = stringText.charAt(i);
                out.print(characterPosition);
            }
        }
