...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        //If statement to check if the indices are not the same.
        if (i != j) {
            //If they are not, then swap the elements of each index.
            T temp = array[i];
            array[i] = array[j];
            array[j] = temp;
        }
    ...
    private static <T> void siftDown(T[] array, int top, int last, Comparator<T> order) {
        ...
        //Initialize the indices of the left and right children of the root.
        int leftIndex = 2 * top + 1;
        int rightIndex = 2 * top + 2;
        //Checking if the leftIndex is smaller than the last element.
        if (leftIndex <= last) {
            //If it is, then the smallerIndex is initialized and is set to the
            //left index.
            int smallerIndex = leftIndex;
            //Check if the heap has a right child.
            if (rightIndex <= last) {
                //If so, then compare to find the smaller index.
                if (order.compare(array[rightIndex], array[smallerIndex]) < 0) {
                    smallerIndex = rightIndex;
                }
            }
            //If the root is larger than the smaller index, recursively call
            //sift down on the subtree.
            if (order.compare(array[top], array[smallerIndex]) > 0) {
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        //Initialize the left, right, and last indices.
        int leftIndex = 2 * top + 1;
        int rightIndex = 2 * top + 2;
        int lastIndex = array.length - 1;

        //If the left index is part of the heap, then recursively call heapify.
        if (leftIndex <= lastIndex) {
            heapify(array, leftIndex, order);
            //If the right index is part of the heap, then recursively call heapify.
            if (rightIndex <= lastIndex) {
                heapify(array, rightIndex, order);
            }
            //At the end, call siftDown to sift down the root.
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        //For every element in q, dequeue and place em in the heap.
        for (int i = 0; i < heap.length; i++) {
            heap[i] = q.dequeue();
        }

        //Call the method heapify to heapify the heap.
    ...
    public final void add(T x) {
        ...
        //Adds the element x to the sortingMachine.
    ...
    public final void changeToExtractionMode() {
        assert this.isInInsertionMode() : "Violation of: this.insertion_mode";
        //Start off by setting insertion mode to false.
        this.insertionMode = false;
        //Build a heap array from this.entries and return it to this.heap.
    ...
    public final T removeFirst() {
        ...
        //Initialize a variable remove that will hold the first element of the
        //heap.
        T remove = this.heap[0];
        //Swap the root of the heap with the element at the end of the heap.
        exchangeEntries(this.heap, 0, this.heapSize - 1);
        //Decrement the heap size.
        this.heapSize--;
        //Sift the root down which will not include T remove.
    ...
    public final int size() {
        ...
        //Start of by initialing the size as 0;
        int size = 0;
        //If insertionMode, let size be the length.
        if (this.insertionMode) {
            size = this.entries.length();
        } else {
            //If not, let the size be the heap size.
            size = this.heapSize;
        }
